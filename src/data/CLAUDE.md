...
#### `shopStockStore.ts`
- **Purpose**: Persists the actual six-item stock currently shown in each NPC's shop (Luma/Sable) until a restock action replaces it
- **State**: `stock: Record<ShopId, { slots: { itemId, sold }[], generatedAt, restockType }>`
- **Actions**: `restockShop(shopId, restockType)` / `restockAllShops(restockType)` (generate a fresh batch via `data/shop/shopStockGenerator.ts`, avoiding immediate repeats of the previous batch); `markSlotSold(shopId, itemId)` (purchased one-time items stay visible-but-sold until the next restock, never silently refilled); `ensureInitialStock()` (idempotent -- only generates for shops that don't have stock yet, safe to call on every mount)
- **Integration**: Reads eligibility/weights from `data/shop/` (see below), which itself reads item metadata from `cosmeticsStore`'s catalog and the furniture/floor/wall pattern catalogs, and ownership from `cosmeticsStore`/`housingStore` via `data/shop/shopOwnership.ts`
- **Persistence**: Stock persists across restarts/rerenders/navigation; only regenerated by an explicit restock call, never by a render

#### `data/shop/` (not a store -- a reusable module `shopStockStore.ts` depends on)
- **Purpose**: Shared "which shop can sell this item, how likely, and what does a healthy 6-item refresh look like" system used by both Luma and Sable rather than per-shop duplicate logic
- **`shopTypes.ts`**: `ShopId` (`"luma" | "sable"`, extensible), `ShopStockConfig` (`{ store, weight, naturalRestock?, manualRestock?, availableFrom?, availableUntil? }`) — item-level metadata items opt into, `ShopCatalogItem` — the normalized shape every source catalog gets adapted into
- **`shopCatalog.ts`**: adapts `cosmeticsStore`'s catalog + `FURNITURE_SHOP_CATALOG` + `FLOOR_PATTERN_CATALOG`/`WALL_PATTERN_CATALOG` into `ShopCatalogItem[]`; items with no explicit `shopStock` default to shared/equal weight rather than being excluded
- **`shopEligibility.ts`**: `getEligibleShopItems(shopId, restockType)` — the one eligibility check both shops share (store match, natural/manual restock flags, availability window, already-owned one-time items excluded)
- **`shopStockGenerator.ts`**: `generateShopStock({ shopId, restockType, previousStockIds })` — weighted selection with category-variety/rare-cap/affordability guardrails (see `shopStockRules.ts`'s `SHOP_STOCK_RULES` and `AFFORDABLE_PRICE_MAX`), degrades gracefully instead of failing on a small catalog
- **Content note**: item-level `shopStock` weights are assigned by each shop's *personality* (Luma = cheery/bright, Sable = goth/moody), not by item category or a "vendor specialty" label — see cosmetics/furniture catalog comments for examples
